...
class RemoteTournament(AsyncWebsocketConsumer):
    ...
    async def qualifyboard(self):
        try:
            ...
            log.error(f'exeption in qualifyboard tournament: {e}')
        ...
        try:
            if self.group_name in RemoteTournament.games:
                ...
                for j in range(max +1):
                    dashboard.append ({ 
                        str(key): [value[Enum.ALIAS], value[Enum.SOCKET].data['profile_image']]
                        for key, value in competitors.items() #get by id
